...
/* Map ra and s-register to [4,15]. so that we can check if the
    register in register list is valid.
  e.g.
  ra  - 4
  s0  - 5
    ....
...
{
  unsigned regno;
  /* check if the register format is xreg.  */
  bool use_xreg = **s == 'x';
  ...
  /* skip "white space, white space" pattern.  */
  while (ISSPACE (**s))
    ++ *s;
  if (**s == '}')
    return TRUE;
...
validate_riscv_insn (const struct riscv_opcode *opc, int length)
{
  ...
  for (oparg = opc->args; *oparg; ++oparg)
	...
	case 'j': used_bits |= ENCODE_ITYPE_IMM (-1U); if (oparg[1] == 'i') ++oparg; break;
